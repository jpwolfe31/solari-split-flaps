/*
Program for controller that operates remote boards 
attached to Solari split flap DIN-6 connector

________________

Operates with remote versions beginning with 1-12-24 
________________


added
no hyphen accross dispaly pages
command delay to help with video recording
more phrases

to dos
continue code clean up
consider decreasing delay between letters for faster page displays
consider adding one space indent for 2d part of hyphenated words
fix bug that sometimes repeats phrase display (e.g. @88 sometimes runs two times) 
...
Command structure for remotes is:
  ...
  addresses 91 and 92, 93 and 94, etc are for clocks
  graphics command use remote decoding and selection and 
  therefore use xx as part of the command

Command sutructure for mkr1010 is through Blynk terminal input.  
,aaaa for "aaaa" string to be output (e.g. " flap this...").  
;aaaa adds hyphens for longer words breaking accross rows.
Also has remote pass through commands if command is 4 letters long 
and was not decoded earlier (e.g. "0105", "011s" or "ca04").  More 
details provided in the remote code.
...
MAX487 Serial Bus Transceiver
    ...
    Note - have not added 120 ohm resistor between A and B
...
20 pin connection (CN1)
    ...
    Note - have not added 120 ohm resistors between A and B.
...
MAX478 timing
    MKR starts as a transmitter at 38400 baud rate - up from 9600 earlier
    Remotes start as receivers
    When a remote receives a transmission command, remote has 8ms to transmit
    Remote must then move back to receive mode
    MKR returns to transmit mode after this delay
  ...
  if (strcmp(cmd_str, "cmd") == 0) { // list commands
    ...
    terminal.println("@ra     - display random phrase"); 
    terminal.println("+      - increment all by one");
    terminal.println("aacc   - remote command");
    terminal.println("[xxxx] - generic remote command");
  ...
  if (strcmp(cmd_str, "cmdm") == 0) { // list more commands
    ...
    terminal.println("rfan   - report fan on/off (toggle)");
    terminal.println("cat    - cancel active time");
    terminal.println("status - report op status");
    terminal.println("rst    - reset controller");
  ...
  if (strcmp(cmd_str, "lrc") == 0) { // list remote commands
    terminal.println("0L set letter");
    terminal.println("10 reset");
    terminal.println("11 turn on request! (active low)");
    terminal.println("12 turn off request!");
    ...
    terminal.println("24 set gLed function to on (default on)");
    terminal.println("25 set gLed function to off");
    terminal.println("26 set rLed function to on (default on)");
    terminal.println("27 set rLed function to off");
    terminal.println("3x... increment by 10, 1, 2, etc");
    terminal.println("40 advance 40");
    terminal.println("41 advance 80");
    terminal.println("42 test motor - 40 advances, unless error");
    terminal.println("43 clear motor error count");
    terminal.println("50 increase drive time by 5ms (default 75ms)");
    terminal.println("51 reduce drive time by 5ms");
    terminal.println("52 increase pause time by 5ms (default 50ms)");
    terminal.println("53 reduce pause time by 5ms");
    terminal.println("6L set index position at L (useful on power up)");
    terminal.println("7L set letter for later commit (c0)");
    terminal.println("80 40 advance on later commit");
    terminal.println("81 80 advance on later commit");
    terminal.println("82 if non space, advance 1 on later commit");
    terminal.println("89 commit (usually global commit 0089)");
    terminal.println("90 rpt position, home, index, error");
    terminal.println("91 rpt version of code");
    terminal.println("92 rpt length test - send back 01 - a...z");

    terminal.println("cx0L column commands");
    terminal.println("dx0L");
    terminal.println("ex0L");
    terminal.println("rx0L row commands");
    terminal.println("sx0L");
    terminal.println("wx0L angle commands");
    terminal.println("mx0L misc commands");
    terminal.println("nx0L");
    terminal.println("ox0L");
    terminal.println("On above, 0L to 3x, 40 or 80 for advances"); 
    terminal.println(); // add line feed
    terminal.flush();
    Blynk.run();
    return;
...
char version_str[32] = "V.032024";  // month/day/year
...
thermistorValue = analogRead(thermistorPin); 
// 1700 = 100deg , 1800 = 90deg, 1900 = 80deg  approx
...
void mkrReset()  // runs at end of 5 second mkrReset timer
{
  ...
  Blynk.logEvent("split-flap restarted", String(notification_str)); 
...
void receive_remote_transmission()  
// blocking serial read - up to 31 characters read in 8ms at 38400 baud
{
...
// read serial data for 8ms at 38400 baud - earlier it was 30ms at 9600 baud
delay(8);  // wait for all serial data to buffer
...
BLYNK_WRITE(V3)
{
  ...
  if (strcmp(cmd_str, "cmd") == 0) { // list commands
    ...
    Blynk.run();
    return;
    } // note 52 max terminal width
  if (strcmp(cmd_str, "lrc") == 0) { // list remote commands
    terminal.println("0L set letter");
    terminal.println("10 reset");
    terminal.println("11 turn on Request! (active low)");
    terminal.println("12 turn off Request!");
    terminal.println("20 set gLed function to power (default)") ;
    terminal.println("21 set gLed function to index");
    terminal.println("22 set rLed function to home (default)");
    terminal.println("23 set rLed function to home and index");
    terminal.println("24 set gLed function to on (default)");
    terminal.println("25 set gLed function to off");
    terminal.println("26 set rLed function to on (default)");
    terminal.println("27 set rLed function to off");
    terminal.println("3x... increment by 10, 1, 2, etc");
    terminal.println("40 advance 40");
    terminal.println("41 advance 80");
    terminal.println("42 conduct motor (index) test (40 adv if no err)"); // 48 char wide
    terminal.println("43 clear motor (index) error count");
    terminal.println("44 clear home error count");    
    terminal.println("50 increase drive time by 5ms (default 75ms)");
    terminal.println("51 reduce drive time by 5ms");
    terminal.println("52 increase pause time by 5ms (default 50ms)");
    terminal.println("53 reduce pause time by 5ms");
    terminal.println("6L set index position at L (useful on power up)"); // 47 char wide
    terminal.println("7L set letter for later commit (89)");
    terminal.println("80 40 advance on later commit");
    terminal.println("81 80 advance on later commit");
    terminal.println("82 if non space, advance by 1 on later commit");
    terminal.println("89 commit (usually global commit 0089)");
    terminal.println("90 rpt position, home, index, error");
    terminal.println("91 rpt version of code");
    terminal.println("92 rpt length test - send back 01 - a...z");
    ...
    terminal.println("rx0L row commands");
    terminal.println("wx0L angle commands");
    terminal.println("xx0L");
    terminal.println("yx0L");
    terminal.println("mx0L misc commands");
    terminal.println("nx0L");
    terminal.println("ox0L");
    terminal.println("On above, 0L to 3x, 40 or 80 for advances"); 
 ...
 /* if ((strcmp(cmd_str, "del0") == 0)
    || (strcmp(cmd_str, "del00") == 0)) { 
    cmdDelay = 0; // set delay to 0 sec
    return;
    }
  if ((strcmp(cmd_str, "del5") == 0)
    || (strcmp(cmd_str, "del05") == 0)) {  
    cmdDelay = 5; // set delay to 5 sec
    return;
    }
  if (strcmp(cmd_str, "del10") == 0){
    cmdDelay = 10; // set delay to 10 sec
    return;
    }*/
  ...
  if (strcmp(cmd_str, "rpt") == 0){  // if rpt, get reports of position and errors from all remotes
    ...
    for (int m = 1; m <= (numSplitFlaps + 2); m++){
      ...
      remote_command_str[3] = '9';  // 90 is position and errors
      ...
      if (m == (numSplitFlaps + 1)){terminal.println();} // add blank line in output
      terminal.println(remote_transmission_str); 
      delay(5);  // note - consider further reducing delay time or eliminating - was 10ms earlier
      }
    WDTimer.clear();  // refresh watch dog timer
    terminal.println();
    terminal.flush();
  ...
  if (strcmp(cmd_str, "rptv") == 0){  // if rpt, get report of version from all remotes
    ...
    for (int m = 1; m <= (numSplitFlaps + 2); m++){  // note update based on remote number
      ...
      remote_command_str[1] = s[0];
      remote_command_str[2] = s[1];
      remote_command_str[3] = '9'; // 91 is version
      ...
      terminal.println(remote_transmission_str); 
      delay(5);  // note - consider further reducing delay time or eliminating
      }
    WDTimer.clear();  // refresh watch dog timer
    terminal.println();
    terminal.flush();
  ...
  if (cmd_str[0] == ','){  // if comma, letter string command
    ...
    for (int m = 1; m <= numSplitFlaps; m++){
      ...
      // pad with spaces
      if (m > cmd_length_int - 1) remote_command_str[4] = ' '; // subtract 1 for first ',' 
      remote_command_str[5] = ']';  // end character
      remote_command_str[6] = '\0';  // terminate string
      send_remote_command();
 ...
 /* For testing
  ...
  if (strcmp(cmd_str, "status") == 0) {
    ...
    if (digitalRead(requestPin) == LOW){
      terminal.println("low, or on");
      }
    else{
      terminal.println("high, or off");
      }
   ...
   if (strcmp(cmd_str, "mtr") == 0) {
    lastCommandMillis = currentMillis; // update time out
    relayOn(); // turn on relay
    strcpy(remote_command_str, "[0042]");
    // send command to remote devices
    send_remote_command();
    // send command to time clocks 91, 92 as well
    delay(5);  // Note - consider reducing delay time or eliminating
    strcpy(remote_command_str, "[9142]");
    send_remote_command(); 
    delay(5);  // Note - consider reducing delay time or eliminating
    strcpy(remote_command_str, "[9242]");
    send_remote_command();
    delay8s();
  ...
  // if not decoded earlier, check if correct length for direct remote command
  if (strlen(cmd_str) == 4){ // note - does not include '\0'
    lastCommandMillis = currentMillis; // update time out
    relayOn(); // turn on relay
  ...
  //  or brackets being used
  //int p = cmd_str[0] - '0';  // convert to integer
  //if (((p >= 0) && (p <= 9)) || ((cmd_str[0] == '[') && (cmd_str[5] == ']'))){ 
  //  lastCommandMillis = currentMillis; // update time out
  //  relayOn(); // turn on relay
  //  strcpy(remote_command_str, "[");
  //  strcat(remote_command_str, cmd_str);
  //  strcat(remote_command_str, "]");
    // override if brackets have been added (e.g. for graphics commands)
  //  if ((cmd_str[0] == '[') && (cmd_str[5] == ']')){
  //    strcpy(remote_command_str, cmd_str);
  //    }
    
    // send command
    send_remote_command();
    // recieve remote transmisison if specified by command
    if (remote_command_str[3] == '9' && remote_command_str[4] == '0'){  
      // read staus back     
      // position, home, index, status read back
      serial1Clear();  // clear any serial remnants
      receive_remote_transmission();
      terminal.println(remote_transmission_str); 
      }
    if (remote_command_str[3] == '9' && remote_command_str[4] == '1'){  // version back
      serial1Clear();  // clear any serial remnants
      receive_remote_transmission();
      terminal.println(remote_transmission_str); 
      }
    if (remote_command_str[3] == '9' && remote_command_str[4] == '2'){  // count 1-30 back
      serial1Clear();  // clear any serial remnants
...
for (int i = 0; i <= word_cnt; i++) {
  ...
  // hyphen should not occur on the last row of a page
  // % mod function returns remainder as integer to identify last row
  // note that row 1 is [0]
  if ((row_number + 1) % 4 == 0){  // last line of 4 row page 
    if (word_str[i][row_length - 1] = '-'){ // row length includes '\0'
      //Serial.println("hyphen found at 12");
      // replace current row with blanks and reset word count
      strcpy(row_display_array[row_number], "            ");
      //Serial.println(row_display_array[row_number]);
      i--;  // repreat replaced word next loop
      }
    }
  }
...
void delay500ms(){delay(475); delay25ms();}
void delay600ms(){delay300ms(); delay300ms();}  // after 500ms, run delay25ms() 2x
void delay700ms(){delay400ms(); delay300ms();}
