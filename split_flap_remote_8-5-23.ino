...
add 41 for 40 advance without motor advance checking (which stops motor) 
...
MAX487 Transceiver
    ...
    (note - not being used at present)
...
20 pin connection
(note - need to fix possible misconnection damage by putting +15 in middle)
    ...
    (note - may need to install terminating 120 resistor 
      between A and B on last remote device)
...
rpt string length test max with controller 30 ms wait 
...
int flapEnablePin = 15; // when high, readPin low every other 
                        //   flap position (inc home)
int homeEnablePin = 16; // when high, readPin low at home
