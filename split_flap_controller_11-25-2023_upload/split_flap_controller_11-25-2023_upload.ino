...
if (strcmp(cmd_str, "cmd") == 0) { // print out list of commands
    ...
    terminal.println("rmo   - report motion state (15s)");
    ...
    terminal.println("cssid - change Wifi SSID");
    terminal.println("cpass - change Wifi password");
    terminal.println("cgmto - change GMT offset");
    terminal.println("csthr - change enable start hour");
    terminal.println("cenhr - change enable end hour");
    terminal.println("cactm - change active time(minutes)");
...
// eeprom data for refernece
  //char ssid[20] = "linksys5";
  //char pass[20] = "23456789";
...
char flipCharsHoursDisplay_str[128] = " 1234567890ABCDEFGHIJKLMT";  // 0 is position T - time characters
...
if (currentMillis - previousMillis_displayActive >= 500) { 
  ...
  if ( (((currentMillis - lastMotionMillis) <= (activeTime * 60000)) && (motionEnable == 1)) ||
...
delay(1000);  // add delay for relay time and remote startup time
...
BLYNK_WRITE(V3)
{
  ...
  if (strcmp(cmd_str, "tdset") == 0) { // set time display // likely not neded by itself
    setTimeDisplay();
    delay(500);
    return;
    } 
  if (strcmp(cmd_str, "tdclr") == 0) { // clear time display  // likely not neded by itself
    clearTimeDisplay();
    delay(500);
    return;
    } 
  ...
  if (cmd_str[0] == ','){  // if comma, letter string command
    ...
    for (int m = 1; m <= numSplitFlaps; m++){
      ...
      remote_command_str[6] = '\0';  // terminate string
      send_remote_command();
      delay (75);  // was 500, then 250, then 100 in the beginning
      WDTimer.clear();  // refresh watch dog timer
      }
    return;
    }  
 ...
 /* For testing
  ...
  if (strcmp(cmd_str, "tsw2") == 0) {  //  led blink
    ...
    */
  if (strcmp(cmd_str, "rmo") == 0) {  //  report motion state (15s)
...
// also for blinking link built in LED
void blinkLedWidget() 
{
if (led1.getValue()) {
  led1.off();
